...
a sdílení. Proto nelze metaprogramování přímo využít pro extrakci a sdílení byznysových pravidel.
Lze však využít pro extrakci informací o struktuře programu a pro identifikaci míst,
ve kterých mají být byznysová pravidla aplikována. Využitím tohoto přístupu ve spojení s
aspektově orientovaným programováním, které bude popsáno v následující sekci, je možné
realizovat automatickou integraci sdílených byznysových pravidel.
...
Logování akcí je zpravidla zohledněno v celém systému velmi podobným
způsobem, jako při vytváření objednávky. Aspekt transakce je stejným
...
vystavení faktury a logistiku objednávek. Konvenční přístup přináší
manuální duplikaci logiky, která zohledňuje tyto aspekty.
...
Přístup \gls{ADDA} implikuje speciální úpravu standardní třívrstvé architektury.
