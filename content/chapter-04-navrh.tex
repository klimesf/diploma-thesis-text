...
V join-pointu~\ref{itm:initialization} služba načte všechna byznysová pravidla, která
...
byznysových pravidel. Tento problém lze řešit zavedením konceptu byznysového kontextu, který
...
adresu, by tak mohl dědit od kontextu vytváření faktury. Na obrázku~\ref{fig:context-extension}
...
Tento koncept však přináší několik problémů, které jsou rozebrány v následujících odstavcích.
...
\section{Logické výrazy byznysových pravidel}\label{sec:expressions-design}
...
Rozhraní \code{Expression} je implementováno dvěma výrazy -- \code{VariableReferece}
a \code{IsNotNull} -- které po zavolání metody \code{accept()} zavolají na visitorovi
příslušnou metodu \code{visit()}. Visitor tak může oddělit logiku, která obstarává
převod jednotlivých výrazů do \gls{DSL}.
...
automaticky distribuovány. Pro tento účel bude využit registr byznysových pravidel
...
Při inicializiaci byznysových kontextů jsou nejprve načteny lokálně dostupné kontexty popsané pomocí \gls{DSL}.
...
Jak bylo popsáno v předchozím textu, byznysové kontexty budou podle prefixu přideleny službám,
...
Nástroj pro centrální administraci nejprve načte všechny byznysové kontexty všech služeb v systému.
Následně zobrazí administrátorovi formulář pro úpravu pravidla.
