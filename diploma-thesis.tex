...
they must be addressed consistently. Current approaches are limited when it
comes to dealing with such tangled concerns, requiring manual restatement and
code duplication, leading to higher development and maintenance costs.}
...
of business rules in such systems. Moreover, the thesis provides a case study
showing reduction of manual business rule duplication within an example system
utilizing the proposed approach.}

\vspace{\baselineskip}

\noindent{\textbf{Keywords:} Service oriented architecture, business rules, aspect-oriented programming, separation of concerns}
...
systému díky členění jeho funkcionality do samostatných služeb. Díky tomu lze
...
automatické integrace těchto pravidel. Součástí práce je případová studie, která
prokazuje snížení manuální duplikace na ukázkovém systému využívajícím navržený přístup.}

\vspace{\baselineskip}

\noindent{\textbf{Klíčová slova:} Architektura orientovaná na služby, byznysová pravidla, aspektově orientované programování, oddělení zodpovědností}
...
\input{content/appendix-A-snimky}
%\input{content/appendix-B-tabulky}
\input{content/appendix-C-uzivatelska-prirucka}
\input{content/appendix-D-zkratky}
\input{content/appendix-E-cd}
